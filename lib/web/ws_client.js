...
var ws= typeof module!=='undefined' ? require('../html/ws').ws : ws_mod.ws;
...
(function(mod){
    ...
    this.client = class websocket extends ws{
	...
	process_message(msg){
	    ...
	    if(hndl!==undefined){
		console.log("Handler found! Prcess comd " + msg.cmd);
		hndl.call(this, msg);
	    }
	    else
	...
	com(json_data, bin_data, monitor){
		...
		cli.signal("error", new Error("Error sending websocket message: " + e));
	    }
	}
	...
	send(command, data, p1, p2){
	    ...
	    this.com(json_data, bin_data, monitor);
	}
	...
	query(command, data, p1, p2, p3){
	    ...
	    this.replies[rid]={ cb : reply, mon : monitor};
